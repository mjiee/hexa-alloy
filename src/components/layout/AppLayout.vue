...
<template>
  ...
  <div class="flex flex-col md:flex-row h-screen overflow-hidden">
    <aside class="hidden md:flex flex-col w-64 bg-white shadow-lg h-full">
      ...
      <nav class="flex flex-col flex-1 p-4 space-y-2 overflow-y-auto min-h-0">
        <router-link
          v-for="item in navItems"
          :key="item.id"
          :to="item.id"
          @click="activeTab = item.id"
          class="nav-item flex items-center p-3 rounded-lg flex-shrink-0 transition-all duration-200"
          :class="
            activeTab === item.id ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-100 hover:translate-x-1'
          "
        >
          <component
            :is="item.icon"
